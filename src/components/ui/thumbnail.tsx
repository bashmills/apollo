...
export function Thumbnail({ className, id }: Props) {
  ...
  useEffect(() => {
    ...
    async function load() {
      const value = id ? await window.backend?.requestThumbnail(id) : null;
      if (!mounted) {
        return;
      }
  ...
  return (
    <div className={`${className}`}>
      {src !== undefined && src !== null && <img className="size-full rounded-md object-cover" src={`apollo://thumbnail/${src}`} />}
